...
data "template_file" "php_artisan_schedule" {
  template = file("../_module/ecs/laravel_backend/batch/json/batch_container_overrides.json")
...
# batchを毎分操作するためにcloud watch event で司令している
resource "aws_cloudwatch_event_target" "schedule" {
  rule      = aws_cloudwatch_event_rule.schedule.name
  arn       = var.cluster_arn
  target_id = "batch" # batch の task 定義を書き換える
